...
export class DddelivererService {
    ...
    async patchDelivery(toPatch,id) {
        ...
        if(toPatch.status=="DEL"){
            ...
            await this.delivery_model.updateOne({id:id.id},{actual_deliver_datetime:body.actual_deliver_datetime})
            await this.delivery_model.updateOne({id:id.id},{status:"DEL"})
        }
        else{
            body.status="TRN"
            await this.delivery_model.updateOne({id:id.id},{status:"TRN"})
        }
        
        let url="https://pasd-webshop-api.onrender.com/api/delivery/"+<String>id.id
